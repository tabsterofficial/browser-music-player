...
async function loadState() {
    try {
        ...
        if (playlist.length > 0 && currentTrackIndex < playlist.length) {
            ...
            await sendMessageToOffscreen('load', { url: currentTrackUrl, currentTime, volume });
        }
    } catch (error) {
...
async function playTrack(index) {
    ...
    isPlaying = true;
    await sendMessageToOffscreen(\'play\', { url: currentTrackUrl, currentTime, volume });    await saveState();
